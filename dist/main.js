...
class Game {
    ...
    turnActionsCard(card) {
        const playerActions = new PlayerAction_1.default(this.players[this.playerIndex], this.cards, this.inJail, this.startAmount);
        switch (card.actionType) {
            case 'luck':
                ...
                switch (card.action[randomLuck].type) {
                    ...
                    case 'gotoRail':
                        let count = 0;
                        const railCard = this.getCard(this.players[this.playerIndex].position);
    ...
    turn() {
        if (this.lock && !this.players[this.playerIndex].bankRupted || this.canEnd && !this.players[this.playerIndex].bankRupted) {
            this.lock = false;
            const playerActions = new PlayerAction_1.default(this.players[this.playerIndex], this.cards, this.inJail, this.startAmount);
            const card = this.getCard(this.players[this.playerIndex].position);
            let prisonLaunch;
            this.canTurn = true;
    ...
    checkAction(action, bids, player, amount) {
        const playerActions = new PlayerAction_1.default(this.players[this.playerIndex], this.cards, this.inJail, this.startAmount);
        const card = this.getCard(this.players[this.playerIndex].position);
        const auctionCard = this.getCard(this.players[this.playerIndex].position);
        let paid;
        switch (action) {
            case 'pay jail fee':
